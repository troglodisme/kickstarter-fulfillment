...
const VARIANT_MAPPING = {
  ...
  'ambientone_black': {
    ...
    price: 159.00, // Base £139 + Black anodising £20
...
app.get('/api/customer/:token', (req, res) => {
  try {
    ...
    const pledgedItemsWithPrices = customer.pledgedItems.map(item => {
      ...
      const priceMap = {
        '52337643290957': 159.00, // Ambient One (Black) - includes £20 black anodising
...
// Test endpoint to generate a working fulfillment link
app.get('/generate-test-link', (req, res) => {
  const customers = Array.from(fulfillmentSystem.customers.values());
  if (customers.length === 0) {
    return res.json({ error: 'No customers loaded' });
  }
  
  const customer = customers[0]; // Get first customer
  const testDiscountCode = 'TEST_DEMO_CODE';
  const token = Buffer.from(`${customer.backerId}_${testDiscountCode}`).toString('base64');
  ...
  res.json({
    ...
    token: token,
    fulfillmentUrl: `${CONFIG.baseUrl}/fulfillment/${token}`,
    message: 'Use this URL to test the customer fulfillment page'
  });
});
